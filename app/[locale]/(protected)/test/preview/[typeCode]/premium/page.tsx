...
import { generateFreeResults, generatePremiumReport } from "@/lib/personality";
...
export default async function PremiumPreviewPage({ params }: PageProps) {
  ...
  const free = generateFreeResults(config.testType, config.scores);
  const premium = generatePremiumReport(config.testType, config.scores);

  if (!free || !premium) notFound();
