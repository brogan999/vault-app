...
/** Data-entry/astrology tests: no progress bar or 3-step boxes (just entering birth data). */
function isDataEntryTest(testId: string): boolean {
  return ESOTERIC_BIRTH_DATA_TEST_IDS.includes(testId as (typeof ESOTERIC_BIRTH_DATA_TEST_IDS)[number]);
}
...
export default function QuestionsPage({ params }: QuestionsPageProps) {
  ...
  const dataEntryOnly = product && testDef ? isDataEntryTest(testId) : false;
  ...
  /** Past result for this test (to show "View your latest results" link). */
  ...
  // Only show email/gender collect form on first test (user has 0 completed tests) or if not signed up
  useEffect(() => {
    getUserTestResults()
      .then((rows) => setCompletedTestCount(rows.length))
      .catch(() => setCompletedTestCount(0)); // show form if we can't determine (e.g. guest)
  }, []);

  // Past results for this test (for "View your latest results" link)
  useEffect(() => {
    getUserTestResults(testId)
      .then((rows) => setLatestResultId(rows.length > 0 ? rows[0].id : null))
      .catch(() => setLatestResultId(null));
  }, [testId]);
  ...
  }, [answers, isSubmitting, questions, testId, collectEmail, collectGender, collectMarketing]);
    ...
    <TestShell>
      ...
      <div className="text-center mb-8">
        ...
        {latestResultId && (
          <p className="mt-3">
            <Link
              href={`/test/${testId}/results/${latestResultId}`}
              className="text-base text-primary hover:underline"
            >
              {tTest("viewLatestResults")}
            </Link>
          </p>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-16">
          {steps.map((step) => (
            ...
            >
              ...
              <h2 className={`text-base font-semibold mb-1 ${step.titleColor}`}>{step.title}</h2>
              <p className={`text-sm leading-relaxed ${step.descColor}`}>{step.description}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex flex-col gap-16">
        {questionsOnPage.map((q, i) => (
          <div
            ...
            className="scroll-mt-4 rounded-2xl border border-border/60 bg-card p-8"
          >
            <TestQuestion
              ref={(r) => {
      ...
      {pageIndex === pageCount - 1 && completedTestCount === 0 && (
        ...
        >
          ...
          <div className="space-y-2">
            <Label className="font-medium text-foreground">{t("genderLabel")}</Label>
